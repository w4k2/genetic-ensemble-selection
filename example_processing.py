...
# Prepare dataset
X, y = make_classification(n_samples=200,n_features=10, weights=(.8,.2),random_state=13)
...
for metric in m:
    ...
    for fold, (train, test) in enumerate(skf.split(X, y)):
        ...
        # Build model LR
        start = time.time()
        clf = GES(metric = metric, base_clf=LogisticRegression(solver="lbfgs"), num_iter=100, beta = .1, alpha=.1)
